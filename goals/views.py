...
def add_goal(request):
    if request.method == 'POST':
        form = GoalForm(request.POST)
        if form.is_valid():
            goal = form.save(commit=False)  # commit=False se pehle save nahi hota
            goal.owner = request.user       # owner ab set ho raha
            goal.save()                     # finally save
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if additional_amount > amount_required:
                ...
                if goal.current_saved_amount >= goal.amount_to_save:
                    goal.current_saved_amount = goal.amount_to_save
                    goal.is_achieved = True
                    goal.save()
                    send_congratulatory_email(request.user.email, goal)
                    messages.success(request, 'Congratulations! You have achieved your goal.')
                    ...
                    goal.save()
                    messages.success(request, f'Amount successfully added. Total saved amount: {goal.current_saved_amount}.')
                    messages.info(request, f'Amount required to reach goal: {goal.amount_to_save - goal.current_saved_amount}.')
...
def delete_goal(request, goal_id):
    try:
        ...
        messages.error(request, 'Goal not found or you do not have permission.')
    return redirect('list_goals')